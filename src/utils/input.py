...
def input_scan_list(input_root, output_root, input_paths=[], output_paths=[]):
    ...
    for content in input_root_content:
        ...
        # if content is dir, recursively call this function to scan dir content
            ...
            input_scan_list(new_input_root, new_output_root, input_paths, output_paths)

    return sorted(input_paths), sorted(output_paths)

def input_scan_dict(input_root, output_root, input_paths=defaultdict(list), output_paths=defaultdict(list)):
    # scan input root dir (find files and subdirs)
    input_root_content = os.listdir(input_root)
